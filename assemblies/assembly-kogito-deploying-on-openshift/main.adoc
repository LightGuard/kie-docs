[id='assembly-kogito-deploying-on-openshift']

include::_artifacts/document-attributes.adoc[]

= Deploying {PRODUCT} services on {OPENSHIFT}
// Context attribute is assembly specific and enables module reuse between assemblies.
:doctype: book
:imagesdir: _images
:context: kogito-deploying-on-openshift
include::_artifacts/author-group.adoc[]

// Purpose statement for the assembly
As a developer of business processes and decisions, you can deploy {PRODUCT} services on {OPENSHIFT} for cloud implementation. The {PRODUCT} Operator automates many of the deployment steps for you or guides you through the deployment process. You can use the {PRODUCT} command-line interface (CLI) to interact with the {PRODUCT} Operator for deployment tasks.

[IMPORTANT]
====
{PRODUCT} is currently available for Development Preview. Development Preview releases contain features and APIs that might not be fully tested and that might change in the final GA version. Users are discouraged from using Development Preview software in production or for business-critical workloads.  Because this is not production-ready software, users are not able to open support tickets. To learn more about {PRODUCT}, please contact your Red Hat representative or send an email to Kogito-earlyaccess@redhat.com. Red Hat will address reported issues at its own discretion.
====

.Prerequisites
* {OPENSHIFT} 4.3 or later is installed.
* The OpenShift project for the deployment is created.

// Modules - concepts, procedures, refs, etc.
include::modules/openshift/con-kogito-on-ocp.adoc[leveloffset=+1]
include::modules/openshift/proc-kogito-deploying-on-ocp-console.adoc[leveloffset=+1]
include::modules/openshift/proc-kogito-deploying-on-ocp-kogito-cli.adoc[leveloffset=+1]
include::modules/openshift/con-kogito-travel-agency.adoc[leveloffset=+1]
include::modules/openshift/proc-kogito-travel-agency-clone-repo.adoc[leveloffset=+2]
include::modules/openshift/proc-kogito-travel-agency-configure-ocp.adoc[leveloffset=+2]
include::modules/openshift/proc-kogito-travel-agency-create-ocp-project.adoc[leveloffset=+2]
include::modules/openshift/proc-kogito-travel-agency-enable-persistence.adoc[leveloffset=+2]
include::modules/openshift/proc-kogito-travel-agency-enable-messaging.adoc[leveloffset=+2]
include::modules/openshift/proc-kogito-travel-agency-enable-data-index.adoc[leveloffset=+2]
include::modules/openshift/proc-kogito-travel-agency-create-services.adoc[leveloffset=+2]
include::modules/openshift/proc-kogito-travel-agency-deploy-binary.adoc[leveloffset=+2]
include::modules/openshift/proc-kogito-travel-agency-interacting.adoc[leveloffset=+2]

== Additional resources
* {URL_CREATING_RUNNING}[_{CREATING_RUNNING}_]
* {URL_DECISION_SERVICES}[_{DECISION_SERVICES}_]
* {URL_PROCESS_SERVICES}[_{PROCESS_SERVICES}_]
* {URL_CONFIGURING_KOGITO}[_{CONFIGURING_KOGITO}_]

// Versioning info
include::_artifacts/versioning-information.adoc[]
